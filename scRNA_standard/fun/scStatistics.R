...
addGeneAnno <- function (gene.manifest, species = "human"){
    ...
    if(species == "human"){
        ...
        diss.genes <- read.table(system.file("txt", "diss-genes.txt", package = "singlercell2"),
    ...
    annotation[gene.manifest$Symbol %in% hb.genes] <- "HB"
...
prepareData <- function(samplePath,
    ...
    if(datatype == "10X"){
        ...
        gene.manifest <- data.frame(EnsemblID = rownames(expr.data),Symbol = rownames(expr.data))
        gene.manifest$Type <- "Gene Expression"
        gene.manifest$Symbol <- as.character(gene.manifest$Symbol)
        gene.manifest$Symbol <- make.unique(gene.manifest$Symbol)
        cr.version <- "BD"
    }
    ...
    geneType <- c("mitochondrial", "ribosome", "dissociation","HB")
    for(tt in geneType) {
        cur.genes <- subset(gene.manifest, Annotation == tt, select = c("Symbol"))$Symbol
		if(length(cur.genes)>1){
        cur.percent <- Matrix::colSums(expr.data[cur.genes,]) / Matrix::colSums(expr.data)
		}else if(length(cur.genes)==1){
		cur.percent <- expr.data[cur.genes,] / Matrix::colSums(expr.data)
		}else{ 
		cur.percent <- 0
		}
...
calcThres <- function(cell.manifest, values = c("nUMI", "nGene"),coef=1.5){
    high.thresholds <- list()
    for(value in values){
        outliers <- getOutliers(cell.manifest[[value]],coef=coef)
        high.thresholds[[value]] <- round(min(outliers), 3)
		if(value=="nGene"){
		     if(high.thresholds[[value]]<5000){
			     high.thresholds[[value]] <- 5000
			 }
		}
		if(value=="mito.percent"){
		    if(high.thresholds[[value]]<0.2){
			     high.thresholds[[value]] <- 0.2
			}
		}
    }
...
genePropPlot <- function(gene.manifest, expr.frac){
    ...
    rate.df.plot$Annotation <- factor(gene.manifest[as.character(rate.df.plot$variable), ]$Annotation,
                                      levels = c("mitochondrial", "ribosome", "dissociation", "HB", "other"), ordered = T)
    ...
    gene.colors <- c(other = "#d56f6d", ribosome = "#f29721", dissociation = "#65a55d", mitochondrial = "#3778bf",HB="#800080")
    p <- ggplot() + coord_flip() +
...
runScStatistics <- function(dataPath, savePath,
    ...
    if(!dir.exists(file.path(savePath, paste(sampleName,'_qc',sep="")))){
        dir.create(file.path(savePath, paste(sampleName,'_qc',sep="")), recursive = T)
    }
    ...
    rm(all)
    write.table(cell.manifest, file = file.path(savePath, paste(sampleName,"_qc/cellManifest-all0.txt",sep="")),
    ...
    if(raw.data){
        ...
        suppressWarnings(
            ggsave(filename = file.path(savePath, paste(sampleName,"_qc/cells-distr-hist.png",sep="")),
                   p.cells.1, dpi = 500, height = 3, width = 4)
        )
        suppressWarnings(
            ggsave(filename = file.path(savePath, paste(sampleName,"_qc/cells-distr-rank.png",sep="")),
    ...
    #                            values = c("nUMI", "nGene", "mito.percent", "ribo.percent", "diss.percent"),coef=coef)
    cell.threshold <- calcThres(cell.manifest,
                                values = c("nUMI", "nGene", "mito.percent", "ribo.percent"),coef=coef)
    ...
    ggsave(filename = file.path(savePath, paste(sampleName,"_qc/nUMI-distr.pdf",sep="")),
           p.nUMI, dpi = 500, height = 2.5, width = 4)
    ggsave(filename = file.path(savePath, paste(sampleName,"_qc/nUMI-distr.png",sep="")),
           p.nUMI, dpi = 500, height = 2.5, width = 4)
    ggsave(filename = file.path(savePath, paste(sampleName,"_qc/nGene-distr.pdf",sep="")),
           p.nGene, dpi = 500, height = 2.5, width = 4)
    ggsave(filename = file.path(savePath, paste(sampleName,"_qc/nGene-distr.png",sep="")),
           p.nGene, dpi = 500, height = 2.5, width = 4)
    ...
    ggsave(filename = file.path(savePath, paste(sampleName,"_qc/mito-distr.pdf",sep="")),
           p.mito, dpi = 500, height = 4, width = 4)
    ggsave(filename = file.path(savePath, paste(sampleName,"_qc/ribo-distr.pdf",sep="")),
           p.ribo, dpi = 500, height = 4, width = 4)
    #ggsave(filename = file.path(savePath, paste(sampleName,"_qc/diss-distr.pdf",sep="")),
    #       p.diss, dpi = 500, height = 4, width = 4)
    ggsave(filename = file.path(savePath, paste(sampleName,"_qc/mito-distr.png",sep="")),
           p.mito, dpi = 500, height = 4, width = 4)
    ggsave(filename = file.path(savePath, paste(sampleName,"_qc/ribo-distr.png",sep="")),
           p.ribo, dpi = 500, height = 4, width = 4)
    #ggsave(filename = file.path(savePath, paste(sampleName,"_qc/diss-distr.png",sep="")),
    #       p.diss, dpi = 500, height = 4, width = 4)
    ...
    suppressWarnings(
        ggsave(filename = file.path(savePath, paste(sampleName,"_qc/geneProp.pdf",sep="")),
               p.geneProp, dpi = 500, height = 8, width = 8)
    )
    suppressWarnings(
        ggsave(filename = file.path(savePath, paste(sampleName,"_qc/geneProp.png",sep="")),
               p.geneProp, dpi = 500, height = 8, width = 8)
    )
    if(!is.null(bg.percent) && raw.data){
        ...
        suppressWarnings(
            ggsave(filename = file.path(savePath, paste(sampleName,"_qc/bg-cell-scatter.pdf",sep="")),
                   p.bg.cell, dpi = 500, height = 4, width = 4)
        )
        suppressWarnings(
            ggsave(filename = file.path(savePath, paste(sampleName,"_qc/bg-detect-scatter.pdf",sep="")),
                   p.bg.detect, dpi = 500, height = 4, width = 4)
        )
        suppressWarnings(
            ggsave(filename = file.path(savePath, paste(sampleName,"_qc/bg-cell-scatter.png",sep="")),
                   p.bg.cell, dpi = 500, height = 4, width = 4)
        )
        suppressWarnings(
            ggsave(filename = file.path(savePath, paste(sampleName,"_qc/bg-detect-scatter.png",sep="")),
                   p.bg.detect, dpi = 500, height = 4, width = 4)
        )
    }else{
        p.bg.cell <- NULL
    ...
    if(bool.runSoupx){
        ...
        suppressWarnings(
            ggsave(filename = file.path(savePath, paste(sampleName,"_qc/bg.genes.soupX.png",sep="")),
...
#    )
    filter.thres <- list(
        Index = c("nUMI", "nGene", "mito.percent", "ribo.percent"),
        Low.threshold = c(0, 200, -Inf, -Inf),
        High.threshold = c(cell.threshold$nUMI,
                           cell.threshold$nGene,
                           cell.threshold$mito.percent,
                           cell.threshold$ribo.percent)
    ...
    write.table(gene.manifest, file = file.path(savePath, paste(sampleName,"_qc/geneManifest.txt",sep="")),
                quote = F, sep = "\t", row.names = F)
    write.table(cell.manifest.all, file = file.path(savePath, paste(sampleName,"_qc/cellManifest-all.txt",sep="")),
                quote = F, sep = "\t", row.names = F)
    write.table(filter.thres, file = file.path(savePath, paste(sampleName,"_qc/cell.QC.thres.txt",sep="")),
                quote = F, sep = "\t", row.names = F)
    ...
    if(genReport){
        ...
        if(!dir.exists(file.path(savePath, 'report-sampleName/'))){
            dir.create(file.path(savePath, 'report-sampleName/'), recursive = T)
        }
        suppressWarnings(
            knit(system.file("rmd", "main-scStat.Rmd", package = "singlercell2"), 
